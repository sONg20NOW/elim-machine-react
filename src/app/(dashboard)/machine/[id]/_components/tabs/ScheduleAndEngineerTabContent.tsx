...
import type {
  ...
  MachineProjectEngineerUpdateRequestDtoType,
  MachineProjectScheduleAndEngineerResponseDtoType,
  MachineProjectScheduleUpdateResponseDtoType
} from '@core/types'
import { handleApiError, handleSuccess } from '@core/utils/errorHandler'
import { buildingGradeOption, checkTypeOption, gradeOption } from '@/@core/data/options'
import {
...
const ScheduleAndEngineerTabContent = () => {
  ...
  const { data: participatedEngineers } = useGetParticipatedEngineerList(machineProjectId)

  return participatedEngineers && <ScheduleAndEngineerTabContentInner />
}

const ScheduleAndEngineerTabContentInner = () => {
  const params = useParams()
  const machineProjectId = params?.id as string
  ...
  const { engineers: temp, ...rest } = scheduleData!

  console.log(temp)

  const scheduleForm = useForm<Omit<MachineProjectScheduleAndEngineerResponseDtoType, 'engineers'>>({
    defaultValues: rest
  })

  const [engineers, setEngineers] = useState(participatedEngineers)

  const isDirtyEngineers =
    JSON.stringify(engineers?.filter(v => v.engineerId !== 0)) !== JSON.stringify(participatedEngineers)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isDirtyEngineers) {
        await auth
          .put<{ data: { machineProjectEngineerUpdateResponseDtos: MachineProjectEngineerUpdateRequestDtoType[] } }>(
            `/api/machine-projects/${machineProjectId}/machine-project-engineers`,
            {
              engineers: engineers?.filter(v => v.engineerId !== 0)
            }
          )
          .then(v => v.data.data.machineProjectEngineerUpdateResponseDtos)

        const response = await refetchParticipatedEngineers()
      ...
      if (isDirtySchedule) {
        await scheduleForm.handleSubmit(async data => {
          const scheduleResponse = await auth
            ...
            }>(`/api/machine-projects/${machineProjectId}/schedule`, data)
            .then(v => v.data.data)
  ...
  const handleAppendEngineer = () => {
    ...
    const skeletonEngineer: machineProjectEngineerDetailDtoType = {
      engineerId: 0,
      engineerName: '',
      grade: '',
      gradeDescription: '',
  ...
  const handleDontSave = useCallback(() => {
    scheduleForm.reset()
    setEngineers(participatedEngineers!)
    setIsEditing(false)
    setShowAlertModal(false)
  }, [setIsEditing, scheduleForm, participatedEngineers])
        ...
        <div className={classNames('flex flex-col gap-2', styles.container)}>
          <table style={{ tableLayout: 'fixed' }}>
            ...
            <tbody>
              ...
              <tr>
                <th style={{ padding: '10px 12px', fontWeight: 600, textAlign: 'left' }}>점검종류</th>
                <SelectTd form={scheduleForm} name='checkType' option={checkTypeOption} />
                <th style={{ padding: '10px 12px', fontWeight: 600, textAlign: 'left' }}>건물등급</th>
                <SelectTd form={scheduleForm} name='buildingGrade' option={buildingGradeOption} />
              </tr>
              <tr>
          ...
          <div className={'flex flex-col gap-2'}>
            ...
            >
              ...
              <tbody>
                <tr style={{ background: '#f3f4f6' }}>
                  ...
                  <td colSpan={3} style={{ textAlign: 'right', padding: '10px 12px' }}></td>
                </tr>
                <tr>
            ...
            >
              ...
              <tbody>
                ...
                {(participatedEngineers?.length ?? 0 > 0) ? (
                  <>
                    ...
                    {(participatedEngineers ?? []).map((eng, idx) => (
